...
def dialog():
    filename =  filedialog.askopenfilename(initialdir = "curtis_program - Testing Seperation/QTDESIGNER/outputExcel",title = "Select file",filetypes = (("excel files","*.xlsx"),("all files","*.*")))
...
def populateList():
    print('\npopulate Path:'+os.getcwd()+'\n')
    #os.chdir(r'../ouputExcel')
    print('\nPopulate Path2:'+os.getcwd()+'\n')
    ...
    readbook = load_workbook('curtis_program - Testing Seperation/QTDESIGNER/outputExcel/'+str(fileentry.get()))
...
def finalize():
    ...
    while listboxindex < listbox2.size():
        ...
        listboxindex+=1
        cellref = sheet.cell(row=finalrow_index,column=tab5dayindex)
    readbook.save('curtis_program - Testing Seperation/QTDESIGNER/outputExcel/'+str(fileentry.get()))
